...
├── sources/                                      # Литература по теме
│   ├── *.pdf                                     # Open-access статьи (PDF)
│   ├── *.md                                      # Конспекты paywalled-статей
│   └── download_log.txt                          # Лог загрузки
...
Папка `sources/` содержит собранные для диплома научные статьи и обзоры по темам:
- ML-based fault location / detection / classification
- COMTRADE preprocessing и симметричные составляющие
- CNN / LSTM / ResNet в релейной защите и диагностике сетей

**Формат хранения:**
- `.pdf` — полные тексты open-access статей (Nature, MDPI, arXiv, IEEE OA и др.)
- `.md` — конспекты статей, недоступных для автоматического скачивания (paywall: ScienceDirect, Springer, Wiley). Включают abstract, ключевые выводы, методологию и DOI.
- `download_log.txt` / `download_log2.txt` — трекинг статуса загрузки каждого источника.
...
- `web-search` — правила веб-поиска и сбора литературы
